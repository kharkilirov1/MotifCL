...
- Memory-native training: `nn::CounterStateLinear` (finite-state counter synapse, 0.75 bytes/weight packed 6-bit state, fused in-backward update; Vulkan-native) and `nn::ReversibleBlock` (reversible activations: NoGrad forward stores no activations, backward recovers inputs via inverse-coupling and recomputes through `autograd::IsolatedBackwardScope`; Vulkan-native for Linear/GELU/Counter couplings). See `docs/MEMORY_NATIVE_TRAINING_METHOD.md`. OpenCL-free witnesses: `tests/test_vulkan_reversible.cpp` (grad parity vs reference) and `tests/test_vulkan_memory_native.cpp` (counter recovery ternary-acc 100%; 4-block reversible+counter stack loss 7.60 → 0.064 over 50 steps).

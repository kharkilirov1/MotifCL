...
`motifcl_generate_transformer --jsonl-repl --completion-only` provides a persistent machine REPL for hot inference harnesses. `python tools/motifcl_ollama_server.py --model ./model.gguf --name motifcl-local:latest --port 11435` wraps that REPL in an Ollama-style local HTTP server with `/api/generate`, `/api/chat`, `/api/tags`, `/v1/completions`, and `/v1/chat/completions`; see `docs/OLLAMA_COMPAT_SERVER.md`. The server starts the runner once, waits for readiness, performs a one-token warmup by default, and then serves hot requests without reloading the model.

...
inline float gguf_q4_k_value_n256_base(__global const uchar* B,
                                       int base,
                                       int sub,
                                       int l,
                                       int group) {
...
inline float2 gguf_q4_k_value_n256_pair_base(__global const uchar* B,
                                             int base,
                                             int sub,
                                             int l0,
                                             int group) {
    ...
    return (float2)(scale * ((float)code0) - bias,
...
inline float8 gguf_q4_k_value_n256_oct_base(__global const uchar* B,
                                            int base,
                                            int sub,
                                            int l0,
                                            int group) {
    ...
    int q7 = (int)qs[l0 + 7];
    int high = sub & 1;
    int code0 = high ? (q0 >> 4) : (q0 & 15);
    int code1 = high ? (q1 >> 4) : (q1 & 15);
    int code2 = high ? (q2 >> 4) : (q2 & 15);
    int code3 = high ? (q3 >> 4) : (q3 & 15);
    int code4 = high ? (q4 >> 4) : (q4 & 15);
    int code5 = high ? (q5 >> 4) : (q5 & 15);
    int code6 = high ? (q6 >> 4) : (q6 & 15);
    int code7 = high ? (q7 >> 4) : (q7 & 15);
    float d = gguf_f16_to_f32(gguf_load_u16_le(B, base));
    float dmin = gguf_f16_to_f32(gguf_load_u16_le(B, base + 2));
    float scale = d * ((float)sc);
    float bias = dmin * ((float)mn);
    return (float8)(scale * ((float)code0) - bias,
                    scale * ((float)code1) - bias,
                    scale * ((float)code2) - bias,
                    scale * ((float)code3) - bias,
                    scale * ((float)code4) - bias,
                    scale * ((float)code5) - bias,
                    scale * ((float)code6) - bias,
                    scale * ((float)code7) - bias);
}
...
inline float gguf_q6_k_value_n256_base(__global const uchar* B,
                                       ...
                                       int ql_high,
                                       int scale_off) {
    int ql_byte = (int)B[base + ql_off];
    int low = ql_high ? ((ql_byte >> 4) & 0x0f) : (ql_byte & 0x0f);
    int high = ((((int)B[base + qh_off]) >> qh_shift) & 0x03) << 4;
    int code = (low | high) - 32;
    float d = gguf_f16_to_f32(gguf_load_u16_le(B, base + 208));
...
inline float8 gguf_q6_k_value_n256_oct_base(__global const uchar* B,
                                            int base,
                                            int ql_off,
                                            int qh_off,
                                            int qh_shift,
    ...
    int ql2 = (int)B[base + ql_off + 2];
    int ql3 = (int)B[base + ql_off + 3];
    int ql4 = (int)B[base + ql_off + 4];
    int ql5 = (int)B[base + ql_off + 5];
    int ql6 = (int)B[base + ql_off + 6];
    int ql7 = (int)B[base + ql_off + 7];
    int low0 = ql_high ? ((ql0 >> 4) & 0x0f) : (ql0 & 0x0f);
    int low1 = ql_high ? ((ql1 >> 4) & 0x0f) : (ql1 & 0x0f);
    int low2 = ql_high ? ((ql2 >> 4) & 0x0f) : (ql2 & 0x0f);
    int low3 = ql_high ? ((ql3 >> 4) & 0x0f) : (ql3 & 0x0f);
    int low4 = ql_high ? ((ql4 >> 4) & 0x0f) : (ql4 & 0x0f);
    int low5 = ql_high ? ((ql5 >> 4) & 0x0f) : (ql5 & 0x0f);
    int low6 = ql_high ? ((ql6 >> 4) & 0x0f) : (ql6 & 0x0f);
    int low7 = ql_high ? ((ql7 >> 4) & 0x0f) : (ql7 & 0x0f);
    int high0 = ((((int)B[base + qh_off + 0]) >> qh_shift) & 0x03) << 4;
    int high1 = ((((int)B[base + qh_off + 1]) >> qh_shift) & 0x03) << 4;
    int high2 = ((((int)B[base + qh_off + 2]) >> qh_shift) & 0x03) << 4;
    int high3 = ((((int)B[base + qh_off + 3]) >> qh_shift) & 0x03) << 4;
    int high4 = ((((int)B[base + qh_off + 4]) >> qh_shift) & 0x03) << 4;
    int high5 = ((((int)B[base + qh_off + 5]) >> qh_shift) & 0x03) << 4;
    int high6 = ((((int)B[base + qh_off + 6]) >> qh_shift) & 0x03) << 4;
    int high7 = ((((int)B[base + qh_off + 7]) >> qh_shift) & 0x03) << 4;
    int code0 = (low0 | high0) - 32;
    int code1 = (low1 | high1) - 32;
    int code2 = (low2 | high2) - 32;
    int code3 = (low3 | high3) - 32;
    int code4 = (low4 | high4) - 32;
    int code5 = (low5 | high5) - 32;
    int code6 = (low6 | high6) - 32;
    int code7 = (low7 | high7) - 32;
    float scale = gguf_f16_to_f32(gguf_load_u16_le(B, base + 208)) *
                  ((float)gguf_i8_from_u8(B[base + scale_off]));
    return (float8)(scale * ((float)code0),
                    scale * ((float)code1),
                    scale * ((float)code2),
                    scale * ((float)code3),
                    scale * ((float)code4),
...
#define DEFINE_MATMUL_Q8_QK_ROW4_F32(KERNEL_NAME, VALUE_FN) \
...
__kernel void matmul_q8_q6_k_row4_n256_f32(__global const char* A,
                                           __global const uchar* B,
                                           __global float* C,
                                           int M,
                                           int N,
    ...
    float acc0 = 0.0f, acc1 = 0.0f, acc2 = 0.0f, acc3 = 0.0f;
    for (int k = 0; k < K; ++k) {
        float b = gguf_q6_k_value_n256_base(B, base, ql_off, qh_off, qh_shift, ql_high, scale_off);
        base += base_step;
        float a0 = (float)((int)A[r0 * K + k]);
        acc0 += a0 * b;
        if (vr1) acc1 += ((float)((int)A[r1 * K + k])) * b;
        if (vr2) acc2 += ((float)((int)A[r2 * K + k])) * b;
        if (vr3) acc3 += ((float)((int)A[r3 * K + k])) * b;
    }
    C[r0 * N + col] = acc0 * rs0;
    if (vr1) C[r1 * N + col] = acc1 * rs1;
    if (vr2) C[r2 * N + col] = acc2 * rs2;
...
__kernel void matmul_q8_q6_k_row4x8_n256_lsa_f32(__global const char* A,
                                                 __global const uchar* B,
                                                 __global float* C,
                                                 int M,
                                                 int N,
    ...
    float8 a2 = (float8)(0.0f);
    float8 a3 = (float8)(0.0f);
    for (int k0 = 0; k0 < K; k0 += 64) {
        int kspan = K - k0;
        if (kspan > 64) kspan = 64;
        int entries = 4 * kspan;
        ...
        if (active) {
            int base = base0 + k0 * base_step;
            for (int kk = 0; kk < kspan; ++kk) {
                float8 bq = gguf_q6_k_value_n256_oct_base(B, base, ql_off, qh_off, qh_shift, ql_high, scale_off);
                base += base_step;
                float x0 = (float)((int)a_tile[kk]);
                a0 += x0 * bq;
                if (vr1) { float x = (float)((int)a_tile[kspan + kk]); a1 += x * bq; }
                if (vr2) { float x = (float)((int)a_tile[2 * kspan + kk]); a2 += x * bq; }
                if (vr3) { float x = (float)((int)a_tile[3 * kspan + kk]); a3 += x * bq; }
            }
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
    if (!active) return;
    C[r0 * N + col0 + 0] = a0.s0 * rs0; C[r0 * N + col0 + 1] = a0.s1 * rs0;
    C[r0 * N + col0 + 2] = a0.s2 * rs0; C[r0 * N + col0 + 3] = a0.s3 * rs0;
    C[r0 * N + col0 + 4] = a0.s4 * rs0; C[r0 * N + col0 + 5] = a0.s5 * rs0;
    C[r0 * N + col0 + 6] = a0.s6 * rs0; C[r0 * N + col0 + 7] = a0.s7 * rs0;
    if (vr1) {
        C[r1 * N + col0 + 0] = a1.s0 * rs1; C[r1 * N + col0 + 1] = a1.s1 * rs1;
        C[r1 * N + col0 + 2] = a1.s2 * rs1; C[r1 * N + col0 + 3] = a1.s3 * rs1;
        C[r1 * N + col0 + 4] = a1.s4 * rs1; C[r1 * N + col0 + 5] = a1.s5 * rs1;
        C[r1 * N + col0 + 6] = a1.s6 * rs1; C[r1 * N + col0 + 7] = a1.s7 * rs1;
    }
    if (vr2) {
        C[r2 * N + col0 + 0] = a2.s0 * rs2; C[r2 * N + col0 + 1] = a2.s1 * rs2;
        C[r2 * N + col0 + 2] = a2.s2 * rs2; C[r2 * N + col0 + 3] = a2.s3 * rs2;
        C[r2 * N + col0 + 4] = a2.s4 * rs2; C[r2 * N + col0 + 5] = a2.s5 * rs2;
        C[r2 * N + col0 + 6] = a2.s6 * rs2; C[r2 * N + col0 + 7] = a2.s7 * rs2;
    }
    if (vr3) {
        C[r3 * N + col0 + 0] = a3.s0 * rs3; C[r3 * N + col0 + 1] = a3.s1 * rs3;
        C[r3 * N + col0 + 2] = a3.s2 * rs3; C[r3 * N + col0 + 3] = a3.s3 * rs3;
        C[r3 * N + col0 + 4] = a3.s4 * rs3; C[r3 * N + col0 + 5] = a3.s5 * rs3;
        C[r3 * N + col0 + 6] = a3.s6 * rs3; C[r3 * N + col0 + 7] = a3.s7 * rs3;
    }
}
...
__kernel void KERNEL_NAME(__global const char* A, \
                          ...
                          int block_a) { \
    int col = get_global_id(0); \
    int row0 = get_global_id(1) * 8; \
    if (col >= N || row0 >= M) return; \
    int r0 = row0 + 0, r1 = row0 + 1, r2 = row0 + 2, r3 = row0 + 3; \
    int r4 = row0 + 4, r5 = row0 + 5, r6 = row0 + 6, r7 = row0 + 7; \
    int vr1 = r1 < M, vr2 = r2 < M, vr3 = r3 < M; \
    int vr4 = r4 < M, vr5 = r5 < M, vr6 = r6 < M, vr7 = r7 < M; \
    float rs0 = scales_a[r0]; \
    float rs1 = vr1 ? scales_a[r1] : 0.0f; \
    float rs2 = vr2 ? scales_a[r2] : 0.0f; \
    float rs3 = vr3 ? scales_a[r3] : 0.0f; \
    float rs4 = vr4 ? scales_a[r4] : 0.0f; \
    float rs5 = vr5 ? scales_a[r5] : 0.0f; \
    float rs6 = vr6 ? scales_a[r6] : 0.0f; \
    float rs7 = vr7 ? scales_a[r7] : 0.0f; \
    float acc0 = 0.0f, acc1 = 0.0f, acc2 = 0.0f, acc3 = 0.0f; \
    ...
    for (int k = 0; k < K; ++k) { \
        ...
        if (vr2) acc2 += ((float)((int)A[r2 * K + k])) * b; \
        if (vr3) acc3 += ((float)((int)A[r3 * K + k])) * b; \
        if (vr4) acc4 += ((float)((int)A[r4 * K + k])) * b; \
        if (vr5) acc5 += ((float)((int)A[r5 * K + k])) * b; \
        if (vr6) acc6 += ((float)((int)A[r6 * K + k])) * b; \
        if (vr7) acc7 += ((float)((int)A[r7 * K + k])) * b; \
    } \
    C[r0 * N + col] = acc0 * rs0; \
    if (vr1) C[r1 * N + col] = acc1 * rs1; \
    if (vr2) C[r2 * N + col] = acc2 * rs2; \
    if (vr3) C[r3 * N + col] = acc3 * rs3; \
    if (vr4) C[r4 * N + col] = acc4 * rs4; \
    if (vr5) C[r5 * N + col] = acc5 * rs5; \
    if (vr6) C[r6 * N + col] = acc6 * rs6; \
    if (vr7) C[r7 * N + col] = acc7 * rs7; \
}

DEFINE_MATMUL_Q8_QK_ROW8_F32(matmul_q8_q4_k_row8_f32, gguf_q4_k_value)
DEFINE_MATMUL_Q8_QK_ROW8_F32(matmul_q8_q6_k_row8_f32, gguf_q6_k_value)

#undef DEFINE_MATMUL_Q8_QK_ROW8_F32

__kernel void matmul_q8_q4_k_row8_n256_f32(__global const char* A,
                                           __global const uchar* B,
                                           __global float* C,
                                           int M,
                                           int N,
    ...
    float acc4 = 0.0f, acc5 = 0.0f, acc6 = 0.0f, acc7 = 0.0f;
    for (int k = 0; k < K; ++k) {
        float b = gguf_q4_k_value_n256_base(B, base, sub, l, group);
        base += base_step;
        float a0 = (float)((int)A[r0 * K + k]);
        acc0 += a0 * b;
        if (vr1) acc1 += ((float)((int)A[r1 * K + k])) * b;
...
__kernel void matmul_q8_q6_k_row8_n256_f32(__global const char* A,
                                           __global const uchar* B,
                                           __global float* C,
                                           int M,
                                           int N,
...
__kernel void KERNEL_NAME(__global const char* A, \
    ...
    for (int k = 0; k < K; ++k) { \
        ...
        a00 += x0 * b0; \
        a01 += x0 * b1; \
        if (vr1) { float x = (float)((int)A[r1 * K + k]); a10 += x * b0; a11 += x * b1; } \
        if (vr2) { float x = (float)((int)A[r2 * K + k]); a20 += x * b0; a21 += x * b1; } \
        if (vr3) { float x = (float)((int)A[r3 * K + k]); a30 += x * b0; a31 += x * b1; } \
        if (vr4) { float x = (float)((int)A[r4 * K + k]); a40 += x * b0; a41 += x * b1; } \
        if (vr5) { float x = (float)((int)A[r5 * K + k]); a50 += x * b0; a51 += x * b1; } \
        if (vr6) { float x = (float)((int)A[r6 * K + k]); a60 += x * b0; a61 += x * b1; } \
        if (vr7) { float x = (float)((int)A[r7 * K + k]); a70 += x * b0; a71 += x * b1; } \
    } \
    C[r0 * N + col0] = a00 * rs0; \
    if (vc1) C[r0 * N + col1] = a01 * rs0; \
    if (vr1) { C[r1 * N + col0] = a10 * rs1; if (vc1) C[r1 * N + col1] = a11 * rs1; } \
    if (vr2) { C[r2 * N + col0] = a20 * rs2; if (vc1) C[r2 * N + col1] = a21 * rs2; } \
    if (vr3) { C[r3 * N + col0] = a30 * rs3; if (vc1) C[r3 * N + col1] = a31 * rs3; } \
    if (vr4) { C[r4 * N + col0] = a40 * rs4; if (vc1) C[r4 * N + col1] = a41 * rs4; } \
    if (vr5) { C[r5 * N + col0] = a50 * rs5; if (vc1) C[r5 * N + col1] = a51 * rs5; } \
    if (vr6) { C[r6 * N + col0] = a60 * rs6; if (vc1) C[r6 * N + col1] = a61 * rs6; } \
...
__kernel void matmul_q8_q4_k_row8x2_n256_f32(__global const char* A,
                                             __global const uchar* B,
                                             __global float* C,
                                             int M,
                                             int N,
    ...
    float a60 = 0.0f, a61 = 0.0f, a70 = 0.0f, a71 = 0.0f;
    for (int k = 0; k < K; ++k) {
        float2 bpair = gguf_q4_k_value_n256_pair_base(B, base, sub, l0, group);
        base += base_step;
        float b0 = bpair.x;
        float b1 = bpair.y;
        float x0 = (float)((int)A[r0 * K + k]);
    ...
    C[r0 * N + col0] = a00 * rs0;
    C[r0 * N + col1] = a01 * rs0;
    if (vr1) { C[r1 * N + col0] = a10 * rs1; C[r1 * N + col1] = a11 * rs1; }
    if (vr2) { C[r2 * N + col0] = a20 * rs2; C[r2 * N + col1] = a21 * rs2; }
    if (vr3) { C[r3 * N + col0] = a30 * rs3; C[r3 * N + col1] = a31 * rs3; }
    if (vr4) { C[r4 * N + col0] = a40 * rs4; C[r4 * N + col1] = a41 * rs4; }
...
__kernel void matmul_q8_q4_k_row4x4_n256_lsa_f32(__global const char* A,
                                                 __global const uchar* B,
                                                 __global float* C,
                                                 int M,
                                                 int N,
...
__kernel void matmul_q8_q4_k_row4x8_n256_lsa_f32(__global const char* A,
                                                 __global const uchar* B,
                                                 __global float* C,
                                                 int M,
                                                 int N,
    ...
    for (int k0 = 0; k0 < K; k0 += 64) {
        ...
        if (active) {
            int base = base0 + k0 * base_step;
            for (int kk = 0; kk < kspan; ++kk) {
                float8 bq = gguf_q4_k_value_n256_oct_base(B, base, sub, l0, group);
                base += base_step;
                float x0 = (float)((int)a_tile[kk]);
                a0 += x0 * bq;
                if (vr1) { float x = (float)((int)a_tile[kspan + kk]); a1 += x * bq; }
    ...
    if (!active) return;
    C[r0 * N + col0 + 0] = a0.s0 * rs0;
    C[r0 * N + col0 + 1] = a0.s1 * rs0;
    C[r0 * N + col0 + 2] = a0.s2 * rs0;
    C[r0 * N + col0 + 3] = a0.s3 * rs0;
    C[r0 * N + col0 + 4] = a0.s4 * rs0;
    C[r0 * N + col0 + 5] = a0.s5 * rs0;
    C[r0 * N + col0 + 6] = a0.s6 * rs0;
    C[r0 * N + col0 + 7] = a0.s7 * rs0;
    if (vr1) {
        C[r1 * N + col0 + 0] = a1.s0 * rs1; C[r1 * N + col0 + 1] = a1.s1 * rs1;
        C[r1 * N + col0 + 2] = a1.s2 * rs1; C[r1 * N + col0 + 3] = a1.s3 * rs1;
        C[r1 * N + col0 + 4] = a1.s4 * rs1; C[r1 * N + col0 + 5] = a1.s5 * rs1;
...
__kernel void matmul_q8_q4_k_row8x4_n256_lsa_f32(__global const char* A,
                                                 __global const uchar* B,
                                                 __global float* C,
                                                 int M,
                                                 int N,
    ...
    int base0 = (col0 >> 8) * 144;
    int base_step = (N >> 8) * 144;
    float4 a0 = (float4)(0.0f);
    float4 a1 = (float4)(0.0f);
    float4 a2 = (float4)(0.0f);
    float4 a3 = (float4)(0.0f);
    float4 a4 = (float4)(0.0f);
    float4 a5 = (float4)(0.0f);
    float4 a6 = (float4)(0.0f);
    float4 a7 = (float4)(0.0f);
    for (int k0 = 0; k0 < K; k0 += 64) {
        int kspan = K - k0;
        if (kspan > 64) kspan = 64;
        int entries = 8 * kspan;
        ...
        if (active) {
            ...
            for (int kk = 0; kk < kspan; ++kk) {
                ...
                if (vr2) { float x = (float)((int)a_tile[2 * kspan + kk]); a2 += x * bq; }
                if (vr3) { float x = (float)((int)a_tile[3 * kspan + kk]); a3 += x * bq; }
                if (vr4) { float x = (float)((int)a_tile[4 * kspan + kk]); a4 += x * bq; }
                if (vr5) { float x = (float)((int)a_tile[5 * kspan + kk]); a5 += x * bq; }
                if (vr6) { float x = (float)((int)a_tile[6 * kspan + kk]); a6 += x * bq; }
                if (vr7) { float x = (float)((int)a_tile[7 * kspan + kk]); a7 += x * bq; }
            }
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
    if (!active) return;
    C[r0 * N + col0 + 0] = a0.s0 * rs0; C[r0 * N + col0 + 1] = a0.s1 * rs0; C[r0 * N + col0 + 2] = a0.s2 * rs0; C[r0 * N + col0 + 3] = a0.s3 * rs0;
    if (vr1) { C[r1 * N + col0 + 0] = a1.s0 * rs1; C[r1 * N + col0 + 1] = a1.s1 * rs1; C[r1 * N + col0 + 2] = a1.s2 * rs1; C[r1 * N + col0 + 3] = a1.s3 * rs1; }
    if (vr2) { C[r2 * N + col0 + 0] = a2.s0 * rs2; C[r2 * N + col0 + 1] = a2.s1 * rs2; C[r2 * N + col0 + 2] = a2.s2 * rs2; C[r2 * N + col0 + 3] = a2.s3 * rs2; }
    if (vr3) { C[r3 * N + col0 + 0] = a3.s0 * rs3; C[r3 * N + col0 + 1] = a3.s1 * rs3; C[r3 * N + col0 + 2] = a3.s2 * rs3; C[r3 * N + col0 + 3] = a3.s3 * rs3; }
    if (vr4) { C[r4 * N + col0 + 0] = a4.s0 * rs4; C[r4 * N + col0 + 1] = a4.s1 * rs4; C[r4 * N + col0 + 2] = a4.s2 * rs4; C[r4 * N + col0 + 3] = a4.s3 * rs4; }
    if (vr5) { C[r5 * N + col0 + 0] = a5.s0 * rs5; C[r5 * N + col0 + 1] = a5.s1 * rs5; C[r5 * N + col0 + 2] = a5.s2 * rs5; C[r5 * N + col0 + 3] = a5.s3 * rs5; }
    if (vr6) { C[r6 * N + col0 + 0] = a6.s0 * rs6; C[r6 * N + col0 + 1] = a6.s1 * rs6; C[r6 * N + col0 + 2] = a6.s2 * rs6; C[r6 * N + col0 + 3] = a6.s3 * rs6; }
    if (vr7) { C[r7 * N + col0 + 0] = a7.s0 * rs7; C[r7 * N + col0 + 1] = a7.s1 * rs7; C[r7 * N + col0 + 2] = a7.s2 * rs7; C[r7 * N + col0 + 3] = a7.s3 * rs7; }
}
...
__kernel void KERNEL_NAME(__global const char* A, \
    ...
    for (int k = lid; k < K; k += local_size) { \
        float b = VALUE_FN(B, k * N + col); \
        acc0 += ((float)((int)A[r0 * K + k])) * b; \
        if (vr1) acc1 += ((float)((int)A[r1 * K + k])) * b; \
        if (vr2) acc2 += ((float)((int)A[r2 * K + k])) * b; \
        if (vr3) acc3 += ((float)((int)A[r3 * K + k])) * b; \
    } \
    __local float* red0 = scratch; \
    __local float* red1 = scratch + local_size; \
    __local float* red2 = scratch + 2 * local_size; \
    __local float* red3 = scratch + 3 * local_size; \
    red0[lid] = acc0; \
    red1[lid] = acc1; \
    red2[lid] = acc2; \
    red3[lid] = acc3; \
    ...
    for (int stride = local_size >> 1; stride > 0; stride >>= 1) { \
        if (lid < stride) { \
            red0[lid] += red0[lid + stride]; \
            red1[lid] += red1[lid + stride]; \
            red2[lid] += red2[lid + stride]; \
            red3[lid] += red3[lid + stride]; \
        } \
        barrier(CLK_LOCAL_MEM_FENCE); \
    } \
    if (lid == 0) { \
        C[r0 * N + col] = red0[0] * scales_a[r0]; \
        if (vr1) C[r1 * N + col] = red1[0] * scales_a[r1]; \
        if (vr2) C[r2 * N + col] = red2[0] * scales_a[r2]; \
        if (vr3) C[r3 * N + col] = red3[0] * scales_a[r3]; \
    } \
}

DEFINE_MATMUL_Q8_QK_ROW4_KPAR_F32(matmul_q8_q4_k_row4_kpar_f32, gguf_q4_k_value)
DEFINE_MATMUL_Q8_QK_ROW4_KPAR_F32(matmul_q8_q6_k_row4_kpar_f32, gguf_q6_k_value)

#undef DEFINE_MATMUL_Q8_QK_ROW4_KPAR_F32
...
// RX580/decode-oriented Q4_0 M=1 variant: one wave64 computes four output
// columns with a parallel reduction across K. This keeps the standard row-major
// Q4_0 layout but removes the single-work-item serial-K bottleneck from the
// scalar decode kernel above.
__kernel void matmul_f32_q4_0_scaled_m1_wg64x4_f32(__global const float* A,
                                                   __global const uchar* B,
                                                   __global float* C,
                                                   int N,
                                                   int K,
...
__kernel void matmul_f32_q4_0_m1_2out_wg64x4_f32(__global const float* A,
    ...
    for (int k = lid; k < K; k += 64) {
        ...
        if (gid < total) {
            ...
            acc0 += av * ((float)q4_0_load(B, k * (first ? N0 : N1) + col)) * S[col];
        }
        gid = gid0 + 1;
        if (gid < total) {
            const int first = gid < N0;
            ...
            __global const uchar* B = first ? B0 : B1;
            __global const float* S = first ? S0 : S1;
            acc1 += av * ((float)q4_0_load(B, k * (first ? N0 : N1) + col)) * S[col];
        }
        gid = gid0 + 2;
        if (gid < total) {
            const int first = gid < N0;
            ...
            __global const uchar* B = first ? B0 : B1;
            __global const float* S = first ? S0 : S1;
            acc2 += av * ((float)q4_0_load(B, k * (first ? N0 : N1) + col)) * S[col];
        }
        gid = gid0 + 3;
        if (gid < total) {
            const int first = gid < N0;
            ...
            __global const uchar* B = first ? B0 : B1;
            __global const float* S = first ? S0 : S1;
            acc3 += av * ((float)q4_0_load(B, k * (first ? N0 : N1) + col)) * S[col];
        }
    }
    scratch[lid] = acc0;
    scratch[64 + lid] = acc1;
...
__kernel void matmul_f32_q4_0_m1_3out_wg64x4_f32(__global const float* A,
                                                 __global const uchar* B0,
                                                 __global const float* S0,
                                                 __global const uchar* B1,
                                                 __global const float* S1,
    ...
    for (int k = lid; k < K; k += 64) {
        ...
        if (gid < total) {
            ...
            if (gid >= total01) { col = gid - total01; n = N2; B = B2; S = S2; }
            else if (gid >= N0) { col = gid - N0; n = N1; B = B1; S = S1; }
            acc0 += av * ((float)q4_0_load(B, k * n + col)) * S[col];
        }
        gid = gid0 + 1;
        if (gid < total) {
            int col = gid;
            ...
            acc1 += av * ((float)q4_0_load(B, k * n + col)) * S[col];
        }
        gid = gid0 + 2;
        if (gid < total) {
            int col = gid;
            ...
            acc2 += av * ((float)q4_0_load(B, k * n + col)) * S[col];
        }
        gid = gid0 + 3;
        if (gid < total) {
            int col = gid;
            ...
            acc3 += av * ((float)q4_0_load(B, k * n + col)) * S[col];
        }
    }
    scratch[lid] = acc0;
    scratch[64 + lid] = acc1;
    ...
    if (lid == 0) {
        for (int j = 0; j < 4; ++j) {
            ...
            const float v = scratch[j * 64];
            if (gid < N0) C0[gid] = v;
            else if (gid < total01) C1[gid - N0] = v;
            else C2[gid - total01] = v;
        }
    }
}

__kernel void matmul_f32_q4_0_packed_qkv_m1_wg64x4_f32(__global const float* A,
                                                       __global const uchar* B,
    ...
    for (int k = lid; k < in_dim; k += 64) {
        const float av = A[k];
        int gid = gid0;
        if (gid < total) acc0 += av * ((float)q4_0_load(B, k * total + gid)) * S[gid];
        gid = gid0 + 1;
        if (gid < total) acc1 += av * ((float)q4_0_load(B, k * total + gid)) * S[gid];
        gid = gid0 + 2;
        if (gid < total) acc2 += av * ((float)q4_0_load(B, k * total + gid)) * S[gid];
        gid = gid0 + 3;
        if (gid < total) acc3 += av * ((float)q4_0_load(B, k * total + gid)) * S[gid];
    }
    scratch[lid] = acc0;
    scratch[64 + lid] = acc1;
    scratch[128 + lid] = acc2;
    ...
    if (lid == 0) {
        for (int j = 0; j < 4; ++j) {
            const int gid = gid0 + j;
            if (gid >= total) continue;
            const float value = scratch[j * 64];
            if (gid < q_dim) Q[gid] = value;
            else if (gid < qk_dim) Kout[gid - q_dim] = value;
            else V[gid - qk_dim] = value;
        }
    }
}
...
__kernel void matmul_swiglu_product_f32_q4_0_packed_m1_wg64x4_f32(__global const float* A,
    ...
    for (int k = lid; k < in_dim; k += 64) {
        ...
        int col = col0;
        if (col < hidden) {
            const int up_col = hidden + col;
            gate_acc0 += av * ((float)q4_0_load(B, k * total + col)) * S[col];
            up_acc0 += av * ((float)q4_0_load(B, k * total + up_col)) * S[up_col];
        }
        col = col0 + 1;
        if (col < hidden) {
            const int up_col = hidden + col;
            gate_acc1 += av * ((float)q4_0_load(B, k * total + col)) * S[col];
            up_acc1 += av * ((float)q4_0_load(B, k * total + up_col)) * S[up_col];
        }
        col = col0 + 2;
        if (col < hidden) {
            const int up_col = hidden + col;
            gate_acc2 += av * ((float)q4_0_load(B, k * total + col)) * S[col];
            up_acc2 += av * ((float)q4_0_load(B, k * total + up_col)) * S[up_col];
        }
        col = col0 + 3;
        if (col < hidden) {
            const int up_col = hidden + col;
            gate_acc3 += av * ((float)q4_0_load(B, k * total + col)) * S[col];
            up_acc3 += av * ((float)q4_0_load(B, k * total + up_col)) * S[up_col];
        }
    }
    scratch[lid] = gate_acc0;
    scratch[64 + lid] = gate_acc1;
    scratch[128 + lid] = gate_acc2;
    scratch[192 + lid] = gate_acc3;
    scratch[256 + lid] = up_acc0;
    scratch[320 + lid] = up_acc1;
    scratch[384 + lid] = up_acc2;
    scratch[448 + lid] = up_acc3;
    barrier(CLK_LOCAL_MEM_FENCE);
    for (int stride = 32; stride > 0; stride >>= 1) {
        if (lid < stride) {
            scratch[lid] += scratch[lid + stride];
    ...
    if (lid == 0) {
        ...
        if (col < hidden) {
            const float g = scratch[0];
            C[col] = (g * native_recip(1.0f + native_exp(-g))) * scratch[256];
        }
        col = col0 + 1;
        if (col < hidden) {
            const float g = scratch[64];
            C[col] = (g * native_recip(1.0f + native_exp(-g))) * scratch[320];
        }
        col = col0 + 2;
        if (col < hidden) {
            const float g = scratch[128];
            C[col] = (g * native_recip(1.0f + native_exp(-g))) * scratch[384];
        }
        col = col0 + 3;
        if (col < hidden) {
            const float g = scratch[192];
            C[col] = (g * native_recip(1.0f + native_exp(-g))) * scratch[448];
        }
    }
}

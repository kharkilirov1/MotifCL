...
| 6 | **memory-native backward** (no saved weight, in-kernel grad_w) + truth gate | ✅ узел хранит только `x`; grad_w в регистрах; `tests/test_counter_memory_truth.cpp`. Перф fused-пути перемерить на устройстве (~2× grad_w-GEMM) |
...
## 4. Backward (fused, memory-native) — ядро дизайна

**Реализовано так, что grad_w не материализуется вообще** (даже тайлом): элемент
`grad_w[o,i] = Σ_n grad_out[n,o]·x[n,i]` считается прямо в регистрах OpenCL-ядра из
`grad_out` и `x`. Узел `CounterBackwardNode` хранит **только `x`** — плотный вес не
переносится из forward в backward; в backward он декодируется как временный объект
исключительно ради `grad_x` и тут же уничтожается.

```
backward(grad_out):
    { w = decode(state)·scale            // временный, pre-update
      grad_x = grad_out · w }            // w уничтожается сразу
    NoGrad:                              // апдейт state не пишется в autograd-граф
        counter_row_stats_fused(state, scale, v, grad_out, x, ...) -> scale_new, denom
        counter_apply_update_fused(state, scale, scale_new, denom, grad_out, x, seed, ...)
        commit scale_new
    if x.requires_grad: x.backward(grad_x)
```

Эволюция дизайна: full `[out,in]` grad_w → row-tile `[tile,in]` → **in-kernel (0 буфера)**.
Финальный шаг — текущая реализация; инвариант (≤1 транзиентный плотный вес, 0 dense
grad_w) защищён `tests/test_counter_memory_truth.cpp`. Цена — grad_w считается дважды
(row-stats и apply), ~2× grad_w-GEMM: осознанный размен «компьют за память».

Транзакционная семантика (критично, из раздела 6 research-дока): `grad_x`
считается **старым** весом до того, как `state` обновлён — `grad_x` (и его декод `w`)
полностью посчитан перед `NoGrad`-апдейтом, который трогает только `state`/`scale`.

Замечание по апдейту независимо от входа: узел навешивается, когда autograd включён и
`update_enabled()`, **без** требования `x.requires_grad` — иначе counter-слой на сыром
входе (первый слой) не обновлялся бы. `grad_x` пробрасывается лишь если вход его просит.
